class TinyGesture {
  ...
  onTouchStart(event) {
    ...
    this.disregardVelocityThresholdX = this.options.disregardVelocityThreshold(
      "x",
      this
    );
    this.disregardVelocityThresholdY = this.options.disregardVelocityThreshold(
      "y",
      this
    );
  ...
  onTouchMove(event) {
    ...
    let isMoveBeyondThreshold =
      Math.max(absTouchMoveX, absTouchMoveY) > this.options.pressThreshold;
  ...
  onTouchEnd(event) {
    ...
    if (absX > this.thresholdX || absY > this.thresholdY) {
      ...
      if (this.swipedHorizontal) {
        if (x < 0) {
          if (
            this.velocityX < -this.options.velocityThreshold ||
      ...
      if (this.swipedVertical) {
        if (y < 0) {
          if (
            this.velocityY < -this.options.velocityThreshold ||
      ...
      absX < this.options.pressThreshold &&
      absY < this.options.pressThreshold
    ) {
