...
class InteractionHandler {
  constructor() {
    ...
    instance.evaluateCondition = function (condition, event) {
      ...
      let evaluation = condition
        .pipe(Object.entries)
        .map(([property, value]) =>
          InteractionHandler.conditions[property](value, event)
        );
  ...
  static getModifierStates(event, list = InteractionHandler.modifiers) {
    let states = list
      .map((modifier) => [modifier, event.getModifierState(modifier)])
  ...
  static getAliasForEvent(type) {
    ...
    for (const [alias, plurality] of entries) {
      if (type == alias) return plurality;
    }

    return [type];
  }

  static modifiers = ["Alt", "AltGraph", "Control", "Meta", "Shift"];
  ...
  static conditions = {
    modifiers: function (condition, event) {
      let state = InteractionHandler.getModifierStates(event).pipe(
        Object.entries
      );
      ...
      let result = state
        .filter(([property, value]) => {
          return (
            property.toLowerCase() in condition ||
            InteractionHandler.getAliasForKey(property) in condition
          );
        })
        .map(([property, value]) => {
    ...
    key: function (condition, event) {
      return (
        event.key.toLowerCase() == condition ||
        InteractionHandler.getAliasForKey(event.key) == condition
      );
    },
  };
